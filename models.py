...
class Estudiante(Usuario):
    ...
    def validar_datos(self) -> bool:
        return (super().validar_datos() and 
                len(self._carrera) > 0 and 
                1 <= self._semestre <= 12)
...
class Profesor(Usuario):
    def __init__(self, id: int, nombre: str, correo: str, departamento: str, tipo_contrato: str):
    ...
    def validar_datos(self) -> bool:
        return (super().validar_datos() and 
                len(self._departamento) > 0 and 
                len(self._tipo_contrato) > 0)
...
class Libro(MaterialBibliografico):
    def __init__(self, id: int, titulo: str, autor: str, editorial: str, categoria: str):
...
class Revista(MaterialBibliografico):
    def __init__(self, id: int, titulo: str, autor: str, numero_edicion: int, fecha_publicacion: date):
        super().__init__(id, titulo, autor)
...
class Tesis(MaterialBibliografico):
    def __init__(self, id: int, titulo: str, autor: str, universidad: str, año_defensa: int):
        super().__init__(id, titulo, autor)
...
class Multa:
    ...
    def generar_multa(self):
        print(f"Multa generada: ${self._monto} por {self._dias_retraso} días de retraso")
...
class Catalogo:
    ...
    def buscar_por_palabras_clave(self, palabra_clave: str):
        return [m for m in self._materiales if palabra_clave.lower() in m.titulo.lower()]
    
    def filtrar_por_autor(self, autor: str):
        return [m for m in self._materiales if hasattr(m, '_autor') and autor.lower() in m._autor.lower()]
...
if __name__ == "__main__":
    ...
    profesor = Profesor(2, "Dr. Pérez", "perez@universidad.edu", "Matemáticas", "Tiempo Completo")
    
    libro = Libro(101, "Python para Principiantes", "Autor Python", "Editorial Tech", "Programación")
